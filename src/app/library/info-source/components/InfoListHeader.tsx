...
interface InfoListHeaderProps {
    ...
    sortBy: 'effective_date' | 'created_at';
    setSortBy: (by: 'effective_date' | 'created_at') => void;
    sortOrder: 'desc' | 'asc';
...
export function InfoListHeader({
    ...
    return (
        <header
            className={`shrink-0 w-full px-10 py-5 border-b ${theme.border} flex flex-col gap-3 z-10 backdrop-blur-md bg-opacity-80`}
        >
            <div className="flex items-center justify-between gap-4 min-w-0 w-full">
                <div className="flex items-center gap-3 min-w-0">
                    <div className="flex items-center bg-black/5 dark:bg-white/5 p-1 rounded-full shrink-0">
                        ...
                        >
                            收藏夹{' '}
                            <span className="font-mono text-[11px] font-normal opacity-80">Folders</span>
                        </button>
                        <button
                            type="button"
                        ...
                        >
                            信息条目{' '}
                            <span className="font-mono text-[11px] font-normal opacity-80">Entries</span>
                        </button>
                    </div>
                    {viewMode === 'entries' && activeHubName && (
                        <span className={`text-xs font-bold truncate ${theme.textMuted}`}>
                            · {activeHubName}
                        </span>
                    )}
                </div>
                ...
                <div className="flex items-center gap-3 sm:gap-4 shrink-0 min-w-0">
                    ...
                    >
                        <Plus size={16} /> 录入{' '}
                        <span className="font-mono text-[10px] font-normal opacity-90">Add</span>
                    </button>
                    ...
                    <label
                        ...
                        onClick={() => setPinFavorites(!pinFavorites)}
                    >
                        <div
                            className={`relative w-8 h-5 rounded-full transition-colors ${pinFavorites ? theme.activePill : isStudy ? 'bg-slate-700' : 'bg-stone-300'}`}
                        >
                            <div
                                className={`absolute top-1 left-1 w-3 h-3 bg-white rounded-full transition-transform ${pinFavorites ? 'translate-x-3' : ''}`}
                            />
                        </div>
                        <span
                            className={`text-xs font-bold transition-colors whitespace-nowrap ${pinFavorites ? theme.primary : theme.textMuted} group-hover:${theme.textBase}`}
                        >
                            收藏置顶{' '}
                            <span className="font-mono text-[9px] font-normal opacity-80">Pin ★</span>
                        </span>
                    </label>
                    ...
                    <div className="flex items-center gap-2 min-w-0 max-w-[min(100%,12rem)] sm:max-w-[18rem] md:max-w-none">
                        ...
                        <select
                            ...
                            onChange={(e) => {
                                ...
                                setSortBy(by as 'effective_date' | 'created_at');
                                setSortOrder(order as 'desc' | 'asc');
                            }}
                        >
                            <option value="effective_date-desc">作用时间范围 (最新) ↓</option>
                            <option value="effective_date-asc">作用时间范围 (最早) ↑</option>
