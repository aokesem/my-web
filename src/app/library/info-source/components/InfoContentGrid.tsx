...
import { InfoItem, InfoBookmark, InfoCategory, InfoSourceViewMode } from '../types';
                            ...
                            {allFilteredItems.map((item) => {
                                ...
                                const categoryName =
                                    item.category_ids.length > 0
                                        ? currentCategories.find((c) => c.id === item.category_ids[0])
                                              ?.name
                                        : undefined;
                                ...
                                return (
                                    <InfoCard
                                        ...
                                        onToggleFav={(i: InfoItem) =>
                                            toggleStatus(null, i.id, 'is_favorited')
                                        }
                                        onEdit={handleEditItem}
                                        onDeleteSuccess={fetchData}
                                        onClick={(hub) => onSelectHub(hub.id)}
                                    />
                                );
                            })}
                        </div>
                        {allFilteredItems.length === 0 && (
                            <div className="w-full py-20 flex flex-col items-center justify-center opacity-50">
                                ...
                                <p className={`text-sm ${theme.textMuted} text-center`}>
                                    该筛选下没有收藏夹
                                    <span className="block font-mono text-[11px] mt-1">No folders</span>
                                </p>
                            </div>
                        )}
                    </>
                    ...
                    <>
                        <div className="flex flex-col gap-3">
                            {allFilteredBookmarks.map((bookmark) => {
                                ...
                                const categoryName = bookmark.category_id
                                    ? currentCategories.find((c) => c.id === bookmark.category_id)?.name
                                    : undefined;
                                return (
                                    <BookmarkCard
                                        key={bookmark.id}
                                        ...
                                        categoryName={categoryName}
                                        onToggleFav={(id) =>
                                            toggleBookmarkStatus(null, id, 'is_favorited')
                                        }
                                        onToggleQueue={(id) =>
                                            toggleBookmarkStatus(null, id, 'is_queued')
                                        }
                                        onToggleRead={(id) =>
                                            toggleBookmarkStatus(null, id, 'is_read')
                                        }
                        ...
                        {allFilteredBookmarks.length === 0 && (
                            <div className="w-full py-20 flex flex-col items-center justify-center opacity-50">
                                <Search size={48} className={`mb-4 ${theme.textMuted}`} />
                                <p className={`text-sm ${theme.textMuted}`}>该范围内暂无信息条目</p>
                            </div>
                        )}
                    </>
                )}
